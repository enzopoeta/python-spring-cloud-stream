...
class TestBinder(unittest.TestCase):
    ...
    def test_bind_producer(self):
        ...
        self.channel.exchange_declare.assert_called_with(
            exchange='prefix-destination', exchange_type='topic', durable=True)
        self.channel.queue_declare.assert_any_call(
            queue='prefix-destination.group1', durable=True)
        self.channel.queue_declare.assert_any_call(
            queue='prefix-destination.group2', durable=True)
    ...
    def test_bind_consumer(self):
        ...
        self.channel.exchange_declare.assert_called_with(
            exchange='destination_out', exchange_type='topic', durable=True)
        self.channel.queue_declare.assert_called_with(
            queue='destination_out.group1', durable=True)
